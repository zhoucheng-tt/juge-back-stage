 ...
 * @LastEditTime: 2021-10-22 14:24:35
  ...
  <div class="all">
    ...
    <div class="up">
      <el-form :inline="true"
        ...
        <el-form-item label="车流检测器名称">
          <el-input size="small"
                    style="width: 160px"
                    v-model="upQueryList.deviceName"
                    placeholder="请输入车流检测器名称">
          </el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-dialog id="add"
        ...
        <el-form :inline="true"
          ...
          <el-row style="padding-top: 20px">
            <el-col :span="12">
              <el-form-item label="车流检测器编号:"
                            label-width="160px"
                            prop="deviceId">
                <el-input v-model="addNewScreenList.deviceId"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="车流检测器名称:"
                            label-width="160px"
                            prop="deviceName">
                <el-input v-model="addNewScreenList.deviceName" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
        ...
        <el-form :inline="true"
          ...
          <el-row style="padding-top: 20px">
            <el-col :span="12">
              <el-form-item label="车流检测器编号:"
                            label-width="160px">
                <el-input v-model="editScreen.deviceId"
                          readonly=""></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="车流检测器名称:"
                            label-width="160px">
                <el-input v-model="editScreen.deviceName" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="IP地址:"
...
export default {
  ...
  methods: {
    ...
    queryGuideScreenList () {
      const param = {
        deviceName: this.upQueryList.deviceName,
        pageNum: this.pageNum,
        pageSize: this.pageSize,
      };
      this.$deviceManagement.queryMonitorTrafficList(param).then(res => {
    ...
    onSubmitAdd () {
      this.$refs["newScreen"].validate(valid => {
        if (valid) {
          const param = {
            deviceId: this.addNewScreenList.deviceId,
            deviceName: this.addNewScreenList.deviceName,
            ipAddress: this.addNewScreenList.ipAddress,
            port: this.addNewScreenList.port
          };
          this.$deviceManagement.monitorTrafficAdd(param).then(() => {
    ...
    onSubmitEdit () {
      const param = {
        deviceId: this.editScreen.deviceId,
        deviceName: this.editScreen.deviceName,
        ipAddress: this.editScreen.ipAddress,
        port: this.editScreen.port,
        deviceId: this.editScreen.deviceId
      };
      this.$deviceManagement.monitorTrafficModify(param).then(res => {
    ...
    //删除
    deleteNewScreen (row) {
      this.$confirm("此操作将永久删除该车流检测器, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.idList = [];
        let info = [{ deviceId: row.deviceId }]
        this.$deviceManagement.monitorTrafficDelete(info).then(res => {
    ...
    handleSelectionChange (val) {
      ...
      val.forEach(item => {
        this.idList.push({ deviceId: item.deviceId });
      });
    },
    //批量删除
    batchDelete () {
      if (this.idList === [] || this.idList.length === 0) {
        ...
        this.$confirm("此操作将永久删除该车流检测器, 是否继续?", "提示", {
          ...
          .then(() => {
            this.$deviceManagement.monitorTrafficDelete(this.idList).then(res => {
              this.$message({ type: "success", message: "删除成功!" });
              this.queryGuideScreenList();
            });
          }).catch(() => {
